...
declare
	@trama varchar(max)
= '2026|07|76548887' -- jgarcia (RESPONSABLE DE PRESUPUESTO)    num_idperfil = 2
-- = '2026|07|73539081'  -- gcastro (RESPONSABLE DE CONTABILIDAD)     num_idperfil = 3
-- = '2026|07|09598011'  -- mconstantini (SUPERVISORA)             num_idperfil = 1
-- As
-- Begin
...
110|90|450|500|200¬String|String|String|String|String'
)
...
,tmp001_tasaDetraccion(dato)as(
    ...
    for xml path, type).value('.','varchar
(max)'),1,1,''))
    from tmp001_sep
)
...
select  -- concat(cab.dato,(select r,
    A.cod_estado, est.num_idestado,
    Convert(varchar,A.fec_orden,103), t,  A.txt_numeroorden, t,
    A.txt_motivo, t, B.txt_nombredestino, t, C.txt_accion, t, C.num_idestado
From dbo.mco_cabeceraorden A
cross apply dbo.mco_estado C
...
Where A.txt_anoproceso = pp.anno
    ...
    and case per.num_idperfil when 0 then A.cod_responsable else A.cod_estado end =
...
Order By A.fec_orden, Try_Cast(A.txt_numeroorden as int)
-- for xml path, type).value('.','varchar(max)'),
