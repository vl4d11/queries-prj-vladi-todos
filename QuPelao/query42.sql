...
-- select*from dbo.mpp_afp
-- select text from sys.syscomments where id = object_id('dbo.usp_mpp_Planilla_rp_BoletaCatalogo','p')
-- usp_mpp_Planilla_rp_BoletaCatalogo "12"," "," ","01/28/2026","MRS157","1",0,"99","2025"
declare
    @num_mesgenerado varchar(2),
	@num_mesinicial varchar(2),
	@num_mesfinal varchar(2),
	@fec_emision datetime,
	@cod_trabajador varchar(10),
	@flg_moneda char(1),
	@num_tipocambio decimal(6,3),
	@cod_oficina varchar(2),
	@num_periodo varchar(4)
-- AS
select
    @num_mesgenerado ='01',
	@num_mesinicial =' ',
	@num_mesfinal =' ',
	@fec_emision ='01/01/2026',
	@cod_trabajador
	='VFH183',
	-- ='SPC158',
	-- ='MRS157',
...
A.num_0207, A.num_0208,A.num_0301,
...
update t set
fec_ilicencia = coalesce(nullif(t.fec_ilicencia,''), Convert(Char(10), tt.fec_inicio,103), ''),
fec_flicencia = coalesce(nullif(t.fec_flicencia,''), Convert(Char(10), tt.fec_termino,103), '')
from #tmp001_rpta t
outer apply(select*from dbo.mpp_licencia tt
    where tt.cod_trabajador = t.cod_trabajador and tt.num_periodo = @num_periodo
    and @num_mesgenerado >= Substring(Convert(Char(10), tt.fec_inicio,103),4,2)
    and @num_mesgenerado <= Substring(Convert(Char(10), tt.fec_termino,103),4,2))tt


;with tmp002_rpta as(
    select*from(select cod_trabajador,
    row_number()over(order by cod_trabajador) item,
    rank()over(order by cod_trabajador) rango, fec_iniciovacacion, fec_finvacacion
    from #tmp001_rpta)t where item != rango
)
,tmp003_rpta as(
    select ttt.item, t.cod_trabajador, t.fec_iniciovacacion,
    ...
    order by t.cod_trabajador, ttt.item, t.fec_iniciovacacion offset 0 rows
)
,tmp004_rpta as(
    select distinct tt.item, tt.cod_trabajador, stuff((select ' | ', isnull(inicio, fin)
    from tmp003_rpta t where t.item = tt.item and t.cod_trabajador = tt.cod_trabajador
    order by cod_trabajador, item, fec_iniciovacacion
    for xml path, type).value('.','varchar(30)'),1,3,'') fechas
    from tmp003_rpta tt
)
,tmp004_rpta_Fch_Ini as(
   select*from tmp004_rpta where item = 1
)
,tmp004_rpta_Fch_Fin as(
   select*from tmp004_rpta where item = 2
)
select t.cod_trabajador, t.fechas fec_iniciovacacion, tt.fechas fec_finvacacion
into #tmp001_fecini_fecfin
from tmp004_rpta_Fch_Ini t, tmp004_rpta_Fch_Fin tt
where t.cod_trabajador = tt.cod_trabajador
...
-- usp_mpp_Planilla_rp_BoletaCatalogo "12"," "," ","01/28/2026","999999","1",0,"99","2025"
-- usp_mpp_Planilla_rp_BoletaCatalogo "12"," "," ","01/28/2026","MRS157","1",0,"99","2025"
