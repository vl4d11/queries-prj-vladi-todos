...
select*into #mpp_quincena from dbo.mpp_quincena where 1=2
create table #tmp001_conceptos(
    item  int,
    mes   char(2) collate database_default,
    valor numeric(7,2)
)
create table #tmp002_conceptos(
    item  int,
    mes   char(2) collate database_default,
    valor numeric(7,2)
)
...
insert into #mpp_quincena
select t.*
from dbo.mpp_quincena t cross apply #tmp001_params p
where   t.cod_trabajador = p.cod_trabajador
    and t.txt_anoproceso = p.anio
...
insert into #tmp002_campos
select name campo, stuff(name, 1,4,'') codigo
from sys.columns
where object_id = object_id('dbo.mpp_quincena')
    and name like 'num_%'
    and try_cast(stuff(name, 1,4,'') as int) is not null
order by column_id
...
from(values('01'),('02'),('03'),('04'),('05'),('06'),('07'),('08'),('09'),('10'),('11'),('12'),('13'))m(meses)
...
select t.grupo, '0001' codigo, t.meses, case t.meses when 13
    then sum(tt.tot_grupo_mes)over(partition by t.grupo)
    else isnull(tt.tot_grupo_mes, 0) end tot_grupo_mes
    into #tmp001_grupo
...
from #tmp022_matriz tt where tt.grupo = t.grupo and tt.mes = t.meses)tt
order by t.grupo, t.meses

select t.meses,
    case t.meses when 13 then sum(tt.tot_mes)over() else isnull(tt.tot_mes, 0) end tot_mes
    into #tmp001_mes
from(select distinct meses from #tmp001_mesesGrupo)t outer apply(
    select distinct mes,
    sum(valor)over(partition by mes) tot_mes
from #tmp022_matriz tt where tt.mes = t.meses)tt
order by t.meses
