...
if exists(select 1 from sys.sysobjects where id=object_id('dbo.udf_split2','if'))
...
if exists(select 1 from sys.sysobjects where id=object_id('dbo.udf_splice2','if'))
drop function dbo.udf_splice2
go
create function dbo.udf_splice2(
    @data varchar(max),
    @sepCamp char(1) = '|',
    @sepRegs char(1) = '~'
)returns table as return(
    ...
    ,tmp001_matriz(item, dato)as(
        ...
        from tmp001_data t cross apply dbo.udf_split2(t.dato, @sepCamp))t
